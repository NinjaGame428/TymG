...
class ProductNotifier extends StateNotifier<ProductState> {
  ...
  void onLike() {
    ...
    LocalStorage.setSavedProductsList(_list);
  }
  ...
  Future<void> getProductDetails(
    ...
    _list = LocalStorage.getSavedProductsList();
    ...
    final response = await _productsRepository
        .getProductDetails(productData.uuid.toString());
    response.when(
      success: (data) async {
  ...
  void createCart(BuildContext context, int shopId, VoidCallback onSuccess,
    ...
    if (connected) {
      ...
      dynamic response;
      if (product == null) {
        if (LocalStorage.getCartLocal()
            .where(
              (element) => element.stockId == state.selectedStock?.id,
            )
            .isNotEmpty) {}
        ...
        for (Addons element in state.selectedStock?.addons ?? []) {
          if ((element.active ?? false) && element.quantity != 0) {
            list.add(
              CartRequest(
    ...
    try {
      ...
      final dataShare = {
        "dynamicLinkInfo": {
          ...
          "link": productLink,
          "androidInfo": {
            "androidPackageName": 'com.foodyman.single',
            "androidFallbackLink":
                "${AppConstants.webUrl}/$shopType/$shopId?product=${state.productData?.uuid}"
          },
          "iosInfo": {

...
class HomeNotifier extends StateNotifier<HomeState> {
  ...
  void deleteAddress() {
    LocalStorage.deleteAddressInformation();
    LocalStorage.deleteAddressSelected();
    state = state.copyWith(addressData: null);
  }

  void setAddress([AddressNewModel? data]) async {
    AddressNewModel? addressData = LocalStorage.getAddressSelected();
  ...
  void checkBranch(
    ...
    res.when(success: (shop) async {
      if (LocalStorage.getShopId() != shop.data?.id) {
                ...
                children: [
                  ...
                  Expanded(
                    child: CustomButton(
                      ...
                      textColor: colors.textBlack,
                      onPressed: () {
                        LocalStorage.setShopId(shop.data?.id);
                        state =
                            state.copyWith(selectBranchId: shop.data?.id ?? 0);
  ...
  setInitBranchId() {
    LocalStorage.setShopId(state.selectBranchId);
  ...
  setBranchId(BuildContext context, {int? shopId}) {
    int oldShopId = LocalStorage.getShopId();
  ...
  Future<void> fetchBranches(BuildContext context, bool setDefault) async {
    final connected = await AppConnectivity.connectivity();
    if (connected) {
      state = state.copyWith(isBranchesLoading: true);
      ...
      response.when(
        success: (data) async {
          if (setDefault) {
            LocalStorage.setShopId(data.data?.first.id);
            state = state.copyWith(selectBranchId: data.data?.first.id ?? 0);
  ...
  Future<void> fetchProductsWithCheckBranch(BuildContext context) async {
    ...
    if (connected) {
      ...
      res.when(success: (shop) async {
        ...
        fetchProductsPopular(context,shopId: shop.data?.id);
  ...
  Future<void> fetchProductsPopular(BuildContext context, {int? shopId}) async {
    ...
    if (connected) {
      ...
      final response = await _productsRepository.getProductsPopularPaginate(
        page: 1,
        shopId: shopId ?? LocalStorage.getShopId(),
      );
      response.when(
        success: (data) {
          state = state.copyWith(

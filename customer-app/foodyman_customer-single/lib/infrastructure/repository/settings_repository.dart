...
class SettingsRepository implements SettingsRepositoryFacade {
  ...
  @override
  Future<ApiResult<AdminResponse>> fetchAdminDetail() async {
    try {
      final client = inject<HttpService>().client(requireAuth: true);
      final response = await client.get('/api/v1/dashboard/user/admin-info');
      return ApiResult.success(
        data: AdminResponse.fromJson(response.data),
      );
    } catch (e) {
      debugPrint('==> get Admin detail failure: $e');
      return ApiResult.failure(
        error: AppHelpers.errorHandler(e),
        statusCode: NetworkExceptions.getDioStatus(e),
      );

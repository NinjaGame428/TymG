...
class CartService extends CoreService
{
    ...
    private function cartDetailsUpdate(array $data, Cart $cart): array
    {
        ...
        if (isset($data['first_insert']) && $data['first_insert']) {

            $cartId = $this->firstInsert($data, $cart, $userCart);
    ...
    private function collectProducts(array $data, Cart $cart, UserCart $userCart): int
    {
        try {
            return DB::transaction(function () use ($data, $cart, $userCart) {
                ...
                $parents = $products->whereNull('parent_id')->toArray();
     ...
     * @return int
     */
    private function firstInsert(array $data, Cart $cart, UserCart $userCart): int
    {
        try {
            return DB::transaction(function () use ($data, $cart, $userCart) {
                ...
                foreach ($products as $product) {
                    ...
                    /** @var Stock $stock */
                    $quantity = $this->actualQuantity($stock, $product['quantity']) ?? 0;

                    if ((int)$quantity === 0 || (int)$stock->quantity === 0) {
                        continue;
                    }
                    ...
                    foreach ($product['addons'] as $addon) {
                        ...
                        /** @var Stock $childStock */
                        $childQuantity = $this->actualQuantity($childStock, $addon['quantity']) ?? 0;
...
//		}
    ...
    public function calculateTotalPrice(Cart $cart): bool
    {
        ...
        $bonus = Bonus::where('type',Bonus::TYPE_SUM)
            ?->where('expired_at','>=', now())
            ?->firstWhere('id',$cart->shop?->bonus?->id);

        if ($bonus && $totalPrice < $bonus->value) {
            DB::table('cart_details')
                ->where('stock_id', $bonus->bonus_stock_id)
                ->where('user_cart_id', $cart->userCart?->id)
                ->where('bonus', true)
                ->delete();
        }

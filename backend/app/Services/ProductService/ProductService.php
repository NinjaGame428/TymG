...
class ProductService extends CoreService implements ProductServiceInterface
{
    ...
    public function parentSync(array $data): array
    {
        ...
        $parents  = $data['products'] ?? null;

        if (empty($parents) && isset($data['shop_id'])) {
            $parents = DB::table('products')
                ->when($shopId !== "all", fn (Builder $query) => $query->where('shop_id', $shopId))
                ...
                DB::transaction(function () use ($parentId, $data, &$uuIds) {
                    ...
                    foreach ($stocks as $stock) {
                        ...
                        foreach ($addons as $addon) {
                            ...
                            $addonClone['shop_id']    = $data['shop_id'];
    ...
    public function multipleKitchenUpdate(array $filter): void
    {
        DB::table('products')
